-- ============================================================
-- Migration 084: Update bulk_import_students with extra fields
--
-- Adds support for:
--   guardian_relationship, guardian_address,
--   emergency_contact_name, emergency_contact_phone,
--   emergency_contact_relationship,
--   blood_type, address, city, phone (student's own),
--   current_academic_year (auto-set from p_academic_year),
--   p_default_password (previously sent by frontend but ignored)
-- ============================================================
...
DECLARE
  ...
  v_g_phone        TEXT;
  v_g_email        TEXT;
  v_g_relationship TEXT;
  v_g_address      TEXT;

  -- emergency contact
  v_ec_name         TEXT;
  v_ec_phone        TEXT;
  v_ec_relationship TEXT;
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      v_g_relationship := NULLIF(LOWER(TRIM(v_student->>'guardian_relationship')), '');
      v_g_address      := NULLIF(TRIM(v_student->>'guardian_address'), '');
      v_ec_name        := NULLIF(TRIM(v_student->>'emergency_contact_name'), '');
      v_ec_phone       := NULLIF(TRIM(v_student->>'emergency_contact_phone'), '');
      v_ec_relationship := NULLIF(LOWER(TRIM(v_student->>'emergency_contact_relationship')), '');
      v_gender         := NULLIF(TRIM(v_student->>'gender'), '');
      v_blood_type     := NULLIF(TRIM(v_student->>'blood_type'), '');
      v_phone          := NULLIF(TRIM(v_student->>'phone'), '');
      v_address        := NULLIF(TRIM(v_student->>'address'), '');
      v_city           := NULLIF(TRIM(v_student->>'city'), '');
      ...
      -- Create student record (includes new columns from migration 083)
      INSERT INTO students (
        school_id, registration_number,
        first_name, last_name,
        date_of_birth, gender,
        blood_type, phone, address, city,
        ...
        v_dob, v_gender,
        v_blood_type, v_phone, v_address, v_city,
        v_ec_name, v_ec_phone, v_ec_relationship,
        CURRENT_DATE, v_grade_level, v_class_id, p_academic_year,
        'enrolled'::student_status
      )
      RETURNING id INTO v_student_id;

      -- Create guardian record (with relationship and address)
      INSERT INTO guardians (
        student_id, school_id, relationship,
        full_name, email, phone, address
      ) VALUES (
        v_student_id, p_school_id,
        COALESCE(v_g_relationship, 'guardian'),
        v_g_name, v_g_email, v_g_phone, v_g_address
      );

      -- Enrollment record (active — no application fee required for imports)
      ...
      IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = v_email) THEN
        ...
        INSERT INTO auth.identities (
          ...
          jsonb_build_object(
            'sub', v_auth_id::TEXT, 'email', v_email,
