-- Migration 064: Update bulk_import_students to handle reg_fee_paid per student
-- Run this in: Supabase Dashboard → SQL Editor → New query
--
-- Each student object in the p_students JSON array may now include:
--   reg_fee_paid: true   → registration fee is marked as already paid
--   reg_fee_paid: false  → registration fee is assigned unpaid (default behavior)
--
-- This lets IT Admin / Registrar mark which old students already paid
-- their registration fee in the previous system, so they don't have to
-- visit the finance office again.
...
BEGIN
  ...
  LOOP
    ...
    BEGIN  -- inner block so one row failure doesn't abort the whole batch
      ...
      -- ── Enrollment record — directly active ───────────────────────────────
      ...
      -- ── If reg fee is pre-paid: mark the registration_fee student_fee paid ─
      IF v_reg_fee_paid THEN
        ...
        IF v_reg_fee_id IS NOT NULL THEN
          ...
          -- Record a payment entry for audit trail
