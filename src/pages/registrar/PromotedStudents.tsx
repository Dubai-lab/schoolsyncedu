    ...
    <div className="space-y-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-xs font-medium text-slate-500">
                  ...
                  <th className="px-5 py-2.5 text-left">Promoted</th>
                  ...
                  <tr key={s.student_id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3 text-right">
                      <Button
                        ...
                        title={s.reg_fee_paid ? 'Assign to a class' : 'Bursar must record registration fee payment first'}
                        onClick={() => { setAssigningStudent(s); setSelectedClassId(''); }}
                      >
                        <UserCheck className="h-3.5 w-3.5 mr-1" />
                        {s.reg_fee_paid ? 'Assign Class' : 'Fee Pending'}
                      </Button>
                    </td>
                  </tr>
                ))}
      ...
      {assigningStudent && (
        <Dialog open onClose={() => setAssigningStudent(null)} className="max-w-md">
          <DialogHeader>
            <DialogTitle>Assign Class for {assigningStudent.next_year}</DialogTitle>
          </DialogHeader>
          <DialogBody className="space-y-4">
            <p className="text-sm text-slate-600">
              Assigning <strong>{assigningStudent.first_name} {assigningStudent.last_name}</strong> to a class.
              All fees for that class will be automatically applied.
            </p>
            ...
            <div>
              ...
              {classes.length === 0 && (
                <p className="mt-1.5 text-xs text-amber-600">
                  No classes found. Ask the Principal to create classes for {assigningStudent.next_year} first.
                </p>
              )}
            </div>
          </DialogBody>
